...
export default function CasesSection() {
  const cards = HOME_CASE_TITLES.map((title) =>
    CASES_PAGE.cases.find((item) => item.title === title),
  )
    .filter(Boolean)
    .map((item) =>
      item.title === "자동차 디테일링"
  ...
  return (
    <section
      className="relative flex min-h-[calc(100vh-4.5rem)] items-center overflow-hidden px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24"
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 sm:gap-12 lg:flex-row lg:items-stretch lg:gap-16 xl:gap-20">
        ...
        >
          <div>
            <h2
              id="home-cases-title"
              className="text-balance text-keep mb-[clamp(0.875rem,2vw,1.25rem)] text-[clamp(1.45rem,3.2vw,2.5rem)] font-black leading-[1.12] text-white"
            >
        ...
        {/* 우측: 5개 성공 사례 이미지 패널 */}
        <ScrollReveal name="home-cases-gallery" delay={120} className="flex-1">
                ...
                >
                  <article className="group flex h-full min-h-[clamp(13.25rem,21vw,15.75rem)] flex-col overflow-hidden rounded-xl border border-slate-800/80 bg-slate-950/35 transition-[border-color,transform] duration-300 hover:-translate-y-0.5 hover:border-blue-400/35">
                    ...
                    <div className="flex flex-1 flex-col justify-start px-[clamp(0.875rem,2vw,1.125rem)] py-[clamp(0.75rem,1.7vw,1rem)]">
                      <h3 className="text-keep text-[clamp(0.8125rem,1.65vw,1rem)] font-black leading-snug text-white">
                        {item.title}
                      </h3>
                      <p className="text-keep mt-1 text-[clamp(0.6875rem,1.25vw,0.8125rem)] font-semibold text-slate-500">
                        {item.category}
                      </p>
                    </div>
                  </article>
