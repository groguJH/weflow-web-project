...
function isPastTimeSlot(date, time) {
  if (!date || !isSameDate(date, new Date())) return false;
...
function RequiredBadge() {
  return (
    <span className="inline-flex rounded-full bg-rose-50 px-1.5 py-0.5 text-[0.625rem] font-black leading-none text-rose-600">
      필수
    </span>
  );
}
...
export default function ReservationFormSection() {
  ...
  async function handleSubmit(e) {
    ...
    if (!selectedDate || !selectedTime)
      return alert("날짜와 시간대를 선택하거나 입력해 주세요.");
    if (!formData.agree)
      return alert("개인정보 수집 및 상담 동의에 체크해 주세요.");
  ...
  if (submitted) {
    return (
      <section
        className="min-h-screen pt-28 flex items-center justify-center px-4 relative overflow-hidden"
        aria-live="polite"
      >
        ...
        <article className="relative text-center max-w-md bg-slate-900/70 backdrop-blur-sm border border-white/[0.07] rounded-3xl p-12">
          <div className="w-16 h-16 rounded-full bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center mx-auto mb-6">
            <CheckCircle2
              size="1em"
              className="text-[2rem] text-cyan-400"
              aria-hidden="true"
            />
          </div>
  ...
  return (
    <section
      className="relative min-h-screen pt-24 pb-24 px-4 sm:px-6 lg:px-8 overflow-hidden"
      aria-labelledby="reservation-page-title"
    >
      ...
      <div className="relative max-w-2xl mx-auto">
        <div className="mb-12 flex flex-col items-center text-center">
          <h1
            id="reservation-page-title"
            className="text-3xl font-black leading-tight text-white sm:text-4xl"
          >
            {RESERVATION_PAGE.title}
          </h1>
          <p className="mt-5 text-[clamp(0.82rem,1.6vw,0.95rem)] font-medium leading-relaxed text-slate-400">
            원하시는{" "}
            <span
              className="reservation-subtitle-highlight"
              style={{ "--reservation-subtitle-delay": "0ms" }}
            >
              날짜와 시간대
            </span>
            를 선택해{" "}
            <span
              className="reservation-subtitle-highlight"
              style={{ "--reservation-subtitle-delay": "220ms" }}
            >
              무료상담을 예약하면
            </span>
            ,{" "}
            <span
              className="reservation-subtitle-highlight"
              style={{ "--reservation-subtitle-delay": "440ms" }}
            >
              맞춤견적을 제공합니다
            </span>
            .
        ...
        <div className="space-y-5">
          {/* Step 1: 날짜 선택 */}
          <section
            className="bg-slate-900/70 backdrop-blur-sm border border-white/[0.07] rounded-2xl p-6"
            aria-labelledby="reservation-date-title"
          >
            <div className="flex items-center gap-3 mb-6">
              <StepBadge number="1" />
              <CalendarDays
                size="1em"
                className="text-[1.125rem] text-cyan-400"
                aria-hidden="true"
              />
              <h2
                id="reservation-date-title"
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <Clock
                size="1em"
                className={`text-[1.125rem] ${selectedDate ? "text-cyan-400" : "text-slate-600"}`}
                aria-hidden="true"
              />
              <h2
                id="reservation-time-title"
                className={`flex items-center gap-2 text-base font-bold ${selectedDate ? "text-white" : "text-slate-600"}`}
              >
                시간 선택
                <RequiredBadge />
              </h2>
            ...
            {!selectedDate ? (
              <div className="py-8 text-center">
                <CalendarDays
                  size="1em"
                  className="text-[1.75rem] text-slate-700 mx-auto mb-3"
                  aria-hidden="true"
                />
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-2 min-[420px]:grid-cols-4 sm:grid-cols-5">
                  {timeSlots.map((time) => {
                    const isSelectedSlot = selectedTime === time && !customTime;
                    ...
                    return (
                      <button
                        ...
                        className={`py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 ${
                          disabled
                            ? "cursor-not-allowed bg-slate-900/55 border border-white/[0.04] text-slate-600"
                            : isSelectedSlot
                ...
                <div>
                  ...
                  >
                    {form.timePrefer.label} (담당자가 직접 확인 후 연락드립니다)
                  </label>
                  <input
                    ...
                    placeholder={form.timePrefer.placeholder}
                    className="w-full rounded-xl border border-white/[0.08] bg-slate-800/60 px-4 py-3 text-sm text-white placeholder-slate-600 transition-colors focus:border-cyan-500/50 focus:outline-none"
                  />
                  <p className="mt-2 text-xs leading-relaxed text-slate-500">
                    시간 슬롯이 맞지 않으면 원하는 시간대를 직접 입력해 주세요.
                  </p>
                </div>
              </div>
            )}
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                ...
                />
                <span className="inline-flex flex-wrap items-center gap-1.5 text-xs text-slate-400 group-hover:text-slate-300 transition-colors leading-relaxed">
                  {form.agree}
                  <RequiredBadge />
                </span>
              </label>
