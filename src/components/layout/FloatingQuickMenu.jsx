...
import { BookOpen, CheckCircle2, MessageCircle, Phone } from "lucide-react";
...
function FloatingAction({ action, onOpenDiagnosis, isPanelOpen }) {
  ...
  const content = (
    <>
      <Icon size="1em" className="text-[0.9375rem]" strokeWidth={2.4} aria-hidden="true" />
  ...
  return (
    <>
      <button
        ...
        className="mobile-diagnosis-bar fixed inset-x-4 z-[70] flex cursor-pointer items-center justify-center rounded-[0.875rem] border border-white/[0.16] bg-slate-950/92 px-5 py-3.5 text-sm font-black text-white shadow-[0_1rem_2.5rem_rgba(2,6,23,0.42)] backdrop-blur-md transition-colors hover:bg-slate-900 md:hidden"
        aria-label="무료 진단받기"
      >
        무료 진단받기
      </button>
      ...
      <aside
        ref={menuRef}
        aria-label="빠른 상담 연결"
        className="fixed right-4 bottom-4 z-[70] hidden w-[15.25rem] max-w-[calc(100vw-2rem)] flex-col gap-2.5 md:flex sm:right-6 sm:bottom-6"
      >
        <section
